...
- (ZXBoolArray *)encode:(NSString *)contents error:(NSError **)error {
  if ([contents length] < 2) {
    ...
    if (startsNormal) {
      if (!endsNormal) {
        *error = ZXError(ZXWriterError, [NSString stringWithFormat:@"Invalid start/end guards: %@", contents]);
        return nil;
      }
  ...
  for (int i = 1; i < contents.length - 1; i++) {
    if (([contents characterAtIndex:i] >= '0' && [contents characterAtIndex:i] <= '9') ||
      ...
      *error = ZXError(ZXWriterError, [NSString stringWithFormat:@"Cannot encode : '%C'", [contents characterAtIndex:i]]);
      return nil;
    }
  }
